...
class DC_ElementSets extends DC_Table
{
	...
	{
        if (isset($GLOBALS['TL_DCA'][$this->strTable]['config']['notCreatable']) && $GLOBALS['TL_DCA'][$this->strTable]['config']['notCreatable']) 
		{
			throw new InternalServerErrorException('Table "' . $this->strTable . '" is not creatable.');
        }
        $this->set['type'] = $GLOBALS['TL_LANG']['CTE']['elementset'];
	    ...
	    if (!isset($GLOBALS['TL_DCA'][$this->strTable]['config']['closed']) || 
          (isset($GLOBALS['TL_DCA'][$this->strTable]['config']['closed']) && !$GLOBALS['TL_DCA'][$this->strTable]['config']['closed']))
		{
			$this->set['tstamp'] = 0;
			...
			if ($objInsertStmt->affectedRows)
			{
                ...
                // Add a log entry
				\System::log('A new Elementset will be create in "'.$this->strTable.'".'.$this->getParentEntries($this->strTable, $insertID), __METHOD__, TL_GENERAL);
    ...
    public function createNewElements($intId, $ajaxId, $blnDoNotRedirect)
    {
        ...
        if ($result_newPosition->numRows)
        {
            ...
            while($result_newPosition->next())
            {
                ...
                {
                    if (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]))
                    {
                        // Never copy passwords
                        if (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['inputType']) && $GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['inputType'] == 'password')
                        {
                        ...
                        elseif (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['eval']) && isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['eval']['unique']))
                        {
                        ...
                        elseif (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['eval']) && isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['eval']['doNotCopy']) || isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['eval']) && isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['eval']['fallback']))
                        {
                            $v = \Widget::getEmptyValueByFieldType($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['sql']);
                            ...
                            if (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['eval']['encrypt']))
                            {
        ...
        $this->Database->prepare("DELETE from tl_content WHERE id=?")->execute($_GET["id"]);
        $this->redirect($this->getReferer());
    }
    /**
    ...
    {
        if (isset($GLOBALS['TL_DCA'][$this->strTable]['config']['notEditable']) && $GLOBALS['TL_DCA'][$this->strTable]['config']['notEditable'])
		{
        ...
        if(isset($_POST['filter_reset']) &&  $_POST['filter_reset'] == 1)
        {
            $_POST['category'] = '';
        }
    ...
    {
        if (isset($GLOBALS['TL_DCA'][$this->strTable]['config']['notDeletable']))
		{
		...
		if ($result_newPosition->numRows)
		{
            ...
            if (!$GLOBALS['TL_DCA'][$this->strTable]['config']['closed'])
            {
                ...
                if ($objInsertStmt->affectedRows)
                {
                    ...
                    // Add a log entry
                    \Contao\System('A new entry "'.$this->strTable.'.id='.$insertID.'" has been created by duplicating record "'.$this->strTable.'.id='.$this->intId.'"'.$this->getParentEntries($this->strTable, $insertID), __METHOD__, TL_GENERAL);
                    //$this->redirect($this->getReferer());
